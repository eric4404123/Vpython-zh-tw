...
class GUIDemo():
    ...
    def 建立標籤及按鈕(self):
        ...
        self.執行鍵=  Button(按鈕框, text=" 執行 ", command= self.執行,fg="black", disabledforeground= "#fed", font= btnfont ,bg="#ffc100")
    ...
    def 建立選單列(self):
        ...
        功能選單.add_command(label= "中文化(en->ch)", command= self.英中翻譯, accelerator= 'alt+t')
    ...
    def 建立文字及繪圖區(self):
        ...
        self.text=text= Text(text_frame, name='text', padx=5,wrap='none', width=70,  bg='white', tabs="1"*4,font= tuple(txtfont),undo=True)
        text.name="text"
        ...
        self.text2=text2= Text(text_frame2, name='text', padx=5,wrap='none',height= 50, width=70, bg='white',tabs="1",font= tuple(txtfont),undo=True)
        text.name="text2"
        ...
        scanvas.pack(side=LEFT, fill=BOTH, expand=1)
    ...
    def 執行(self,event=None):
        ...
        import subprocess
        try:
            subprocess.check_output(['python', 'Demotempfile313341.py'])
        except:
            print("出問題啦!!!!!!!!!!!!!!!!!")
            module=""
